...
export const ScheduleModule = ({ defaultViewMode = 'obra' }) => {
  ...
  const handleScheduleSubmit = (e) => {
    e.preventDefault();
    if (!selectedUserIds.length || !startDate || !endDate) return;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div className="glass-panel" style={{ padding: '0.85rem 1.25rem', borderRadius: 'var(--radius-lg)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--bg-main)', padding: '4px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
          ...
          >
            <UserCheck size={15} /> Minha Agenda Pessoal ({myPersonalTrips.length})
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div className="glass-panel" style={{ padding: '1.25rem', borderRadius: 'var(--radius-lg)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem', marginBottom: '0.5rem' }}>
              ...
              <div>
                <span className="badge badge-purple" style={{ marginBottom: '0.15rem' }}>
                  Minha Agenda de Campo & Viagens
                </span>
                <h1 style={{ fontSize: '1.3rem', fontWeight: 800 }}>
                  Agenda Pessoal de {currentUser?.name || 'Usuário'}
                </h1>
                <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                  Exibindo todas as viagens programadas para você em todas as obras.
                </p>
              </div>
            </div>
          </div>
              ...
              {myPersonalTrips.map(trip => (
                <div 
                  ...
                  style={{ 
                    ...
                    borderLeft: `5px solid ${trip.isDoubleCar ? 'var(--accent-amber)' : 'var(--accent-blue)'}`,
                    display: 'flex',
                    flexDirection: 'column',
                    justifyContent: 'space-between',
                    gap: '0.75rem'
                ...
                >
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.35rem' }}>
                      <span className="badge badge-blue">
                        <Briefcase size={12} /> Obra: {trip.obraName}
                      </span>
                      <span className={`badge ${trip.isDoubleCar ? 'badge-amber' : 'badge-emerald'}`}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                      <div>
                        📅 <strong>Período:</strong> {trip.startDate} até {trip.endDate}
                      </div>
                      <div>
                        👥 <strong>Equipe Viajante:</strong> {trip.userNames ? trip.userNames.join(', ') : (trip.userName || currentUser?.name)}
                      </div>
                      <div>
                        📍 <strong>Distância:</strong> {trip.distanceKm || 0} km ida e volta
                      </div>
                      {trip.notes && (
                        <div style={{ padding: '0.5rem', background: 'var(--bg-main)', borderRadius: '4px', border: '1px solid var(--border-color)', fontStyle: 'italic', fontSize: '0.775rem', marginTop: '0.35rem' }}>
                          💬 "{trip.notes}"
                        </div>
                      )}
                    </div>
                  ...
                  <div style={{ fontSize: '0.725rem', color: 'var(--text-muted)', paddingTop: '0.5rem', borderTop: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span>Custo Estimado: <strong>{formatBRL(trip.cost)}</strong></span>
                    <span className="badge badge-emerald">Agendado</span>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="glass-panel" style={{ padding: '3.5rem 1.5rem', textAlign: 'center', borderRadius: 'var(--radius-lg)' }}>
              <Plane size={48} style={{ color: 'var(--text-muted)', marginBottom: '1rem' }} />
              <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '0.4rem' }}>
                Nenhuma Viagem Programada para Você
              </h3>
              <p style={{ fontSize: '0.825rem', color: 'var(--text-secondary)' }}>
                Você ainda não possui viagens agendadas pela administração nas obras ativas.
              </p>
            </div>
          )}
        </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 800, color: modalIsDoubleCar ? 'var(--accent-amber)' : 'var(--accent-blue)', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                      <Car size={16} /> {modalIsDoubleCar ? '🚘🚘 Irão em 2 Carros (> 4 Pessoas)' : '🚘 Irão em 1 Carro (Até 4 Pessoas)'}
                    </div>
